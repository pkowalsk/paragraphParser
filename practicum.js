...
let handleStemWords = wordSet => {
	...
	for(var i = 0; i < wordSet.length; i++){
		var stem = wordSet[i]; // set the word stem equal to the element
...
let getSentencePosition = (wordArray, sentenceArray) => {
...
let result = { results: wordArray };

//creating a json object
let jObject={};
for(i in wordArray)
{
    jObject[i] = wordArray[i];
}

console.log(result);
let prettyJson = JSON.stringify(result, 0, 2);
$('#results').html(prettyJson);
